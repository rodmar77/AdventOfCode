...
object Day15 {
  ...
  def main(args: Array[String]): Unit = {
    val (genA, genB, mod) = (BigInt(16807), BigInt(48271), BigInt(2147483647L))

    val regex = ".+ (\\d+)".r
    val s = Source
      .fromFile("inputs/2017/input_day15.txt")
      .getLines
      .map {
    ...
    println(count(
      s.head,
      a => ((BigInt(a) * genA) % mod).toLong,
      s.last,
      b => ((BigInt(b) * genB) % mod).toLong,
      40000000))
    ...
    println(count(
      s.head,
      a => {
        def nextA(currA: Long): Long = {
          val candidate = ((BigInt(currA) * genA) % mod).toLong
          if (candidate % 4 == 0) candidate
          else nextA(candidate)
        }
      ...
      s.last,
      b => {
        def nextB(currB: Long): Long = {
          val candidate = ((BigInt(currB) * genB) % mod).toLong
          if (candidate % 8 == 0) candidate
          else nextB(candidate)
        }
  ...
  def count(a: Int, genA: Long => Long, b: Int, genB: Long => Long, r: Int) = {
    def count(currA: Long, currB: Long, round: Int, acc: Int): Int = {
      def matchBits() = {
      ...
      if (round == r) acc
      else if (matchBits()) count(genA(currA), genB(currB), round + 1, acc + 1)
      else count(genA(currA), genB(currB), round + 1, acc)
    }

    count(genA(a), genB(b), 1, 0)
  }
}
