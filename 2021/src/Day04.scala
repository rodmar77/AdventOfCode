...
/*
  ...
  appears. (Numbers may not appear on all boards.) If all numbers in any row or any
...
object Day04 {
  ...
  def extractBoards(it: List[String]): List[Board] = {
    @tailrec
    def extractBoards(lines: List[String], acc: List[Board]): List[Board] = lines match {
      case Nil => acc
      case _ :: xs =>
          extractBoards(
            xs.dropWhile(_.nonEmpty),
            acc :+ xs
