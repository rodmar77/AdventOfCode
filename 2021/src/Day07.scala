...
object Day07 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2021/input_day07.txt")) {
      source =>
        val positions = source.bufferedReader().readLine().split(",").map(_.toInt)

        // Determine the horizontal position that the crabs can align to using the least
        // fuel possible. How much fuel must they spend to align to that position?
        val costs = (0 to positions.max).map(m => positions.map(p => (p - m).abs).sum)
        println(costs.min)
         ...
         */
        val costsWithSum = (0 to positions.max).map(m => positions.map(costWitSum(m, _)).sum)
        println(costsWithSum.min)
    }
  }

  def costWitSum(start: Int, end: Int): Int = {
    val an = (end - start).abs
    ((1 + an) * an) / 2
  }
}