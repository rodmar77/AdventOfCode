...
/*
  ...
  that point. The top-left pair of 1s, for example, comes from 2,2 -> 2,1; the very
...
object Day05 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2021/input_day05.txt")) {
      source =>
        ...
        //  two lines overlap?
        println(getHorizontalAndVerticalCrossings(coords).size)

        // Consider all of the lines. At how many points do at least two lines overlap?
        println(getAllCrossings(coords).size)
    }
  }

  private def getAllCrossings(coords: List[((Int, Int), (Int, Int))]) = coords
    .flatMap {
      case ((x0, y0), (x1, y1)) =>
        if (x0 == x1) ((y0 min y1) to (y0 max y1)).map((x0, _))
        else if (y0 == y1) ((x0 min x1) to (x0 max x1)).map((_, y0))
        else {
          val (xinc, yinc) = (if (x1 > x0) 1 else -1, if (y1 > y0) 1 else -1)
          (0 to (x1 - x0).abs).map(k => (x0 + k * xinc, y0 + k * yinc))
        }
    }.groupBy(n => n)
     .view
     .mapValues(_.length)
  ...
  private def getHorizontalAndVerticalCrossings(coords: List[((Int, Int), (Int, Int))]) = coords
    .filter {
      case ((x0, y0), (x1, y1)) => (x0 == x1) || (y0 == y1)
    }.flatMap {
      case ((x0, y0), (x1, y1)) =>
        if (x0 == x1) ((y0 min y1) to (y0 max y1)).map((x0, _))
        else ((x0 min x1) to (x0 max x1)).map((_, y0))
    }.groupBy(n => n)
     .view
     .mapValues(_.length)
     .filter {
