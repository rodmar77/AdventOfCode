...
/*
  ...
  consider.) In this example, the middle score is 288957 because there are the same number
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2021/input_day10.txt")) {
      source =>
        ...
        println(source.getLines().map(getCosts).foldLeft((List[Int](), List[Long]())) {
          case ((la, lb), (a, b)) => (la :+ a, lb :+ b)
        } match {
          case (la, lb) => (la.sum, middle(lb.filter(_ > 0).sorted))
        })
    }
  }

  def middle(ll: List[Long]): Long = ll((ll.length - 1) / 2)

  def getCosts(text: String): (Int, Long) = {
    def matches(a: Char, b: Char) = Set(('(', ')'), ('[', ']'), ('{', '}'), ('<', '>')).contains((a, b))

    @tailrec
    def evaluate(text: List[Char], stack: List[Char]): Any = text match {
      case Nil => stack.map(Map[Char, Char]('('->')','['->']','{'->'}','<'->'>'))
      case c :: cs => c match {
        case '(' | '[' | '{' | '<' => evaluate(cs, c :: stack)
        case _ => if (stack.isEmpty || !matches(stack.head, c)) c else evaluate(cs, stack.tail)
      }
    }

    val errorCost = Map[Char, Int](')' -> 3, ']' -> 57, '}' -> 1197, '>' -> 25137)
    val addCost = Map[Char, Int](')' -> 1, ']' -> 2, '}' -> 3, '>' -> 4)

    evaluate(text.toList, Nil) match {
      case c: Char => (errorCost(c), 0L)
      case l: List[Char] => (0, l.map(addCost).foldLeft(0L) {
        case (total, current) => total*5L + current
      })
    }
  }
}