    ...
    Using(Source.fromFile("inputs/2021/input_day01.txt")) {
      source => val numbers = source.getLines.map(_.toInt).toList
        ...
        /*
          ...
          in the second window is larger than the sum of the first, so this first comparison
          increased.
          ...
          B with C, then C with D, and so on. Stop when there aren't enough measurements
         ...
         */
        println(increaseCount(numbers.sliding(3).map(_.sum).toList))
    }
  }
