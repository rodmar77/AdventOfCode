...
/*
  ...
  wire/segment connections are mixed up separately for each four-digit display! (All of
  ...
  tell which wire (b/g) goes to which segment (c/f). For that, you'll need to collect more
  information.
  ...
  is the only digit that uses four segments, eafb means that to render a 4, signal lines e,
  a, f, and b are on.
...
object Day08 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2021/input_day08.txt")) {
      source =>
        val lines = source.getLines().map(_.split(" \\| ")).map {
          case Array(a, b) => (a.split(" ").toList, b.split(" ").toList)
        }.toList

        // In the output values, how many times do digits 1, 4, 7, or 8 appear?
        println(lines.map {
          case (_, b) => b.count(k => Set(2, 3, 4, 7).contains(k.length))
        }.sum)
        ...
        */
        println(lines.map {
          case (a, b) => toNumber(a, b)
        }.sum)
    }
  }

  def toNumber(segments: List[String], digits: List[String]): Int = {
    def sort(s: String) = s.toSeq.sorted.unwrap
    ...
    digits.map(k => segmentMapping(sort(k))).mkString.toInt
  }
