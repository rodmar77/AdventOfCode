...
/*
  ...
  you're getting out of this cave anytime soon is by finding a path yourself. Not just a
  path - the only way to know if you've found the best path is to find all of them.
  ...
  (written in uppercase, like A) and small caves (written in lowercase, like b). It would
  be a waste of time to visit any small cave more than once, but big caves are large
  enough that it might be worth visiting them multiple times. So, all paths you find
...
object Day12 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2021/input_day12.txt")) {
      source =>
        val matrix = source.getLines().map(_.split("-")).flatMap {
          case Array(a, b) => List((a, b), (b, a))
        }.toList
         .groupBy {
           case (a, _) => a
         }.view
        ...
        // most once?
        println(getDistinctPathsCount(matrix, canRevisitCave = false))
        /*
          --- Part Two ---
         ...
         */
        println(getDistinctPathsCount(matrix, canRevisitCave = true))
    }
  }

  def getDistinctPathsCount(matrix: Map[String, List[String]], canRevisitCave: Boolean): Int = {
    def isSmallCave(cave: String) = cave.forall(Character.isLowerCase)
    def updateVisitCount(cave: String, visitCount: Map[String, Int]) = visitCount + (cave->(visitCount.getOrElse(cave, 0) + 1))
    def exceedsVisitCount(cave: String, visitCount: Map[String, Int]) = {
      if (cave.equals("start")) true
      else if (visitCount.contains(cave)) {
        if (canRevisitCave) visitCount.exists {
          case (k, v) => !k.equals("start") && v > 1
        }
        else true
      } else false
    }

    def getDistinctPathsCount(current: String, visitCount: Map[String, Int]): Int = {
      if (current.equals("end")) 1
      else matrix(current).filterNot(exceedsVisitCount(_, visitCount)).map(
        nextNode =>
          getDistinctPathsCount(
            nextNode,
            if (isSmallCave(nextNode)) updateVisitCount(nextNode, visitCount)
            else visitCount))
        .sum
    }

    getDistinctPathsCount("start", Map[String, Int]("start"->1000))
  }
