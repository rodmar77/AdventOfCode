...
object Day07 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2015/input_day07.txt")) {
      source =>
  ...
  def getValues(lines: List[String]): Map[String, Int] = {
    ...
    case class OneOpWire(f: Int => Int) extends Wire {
      ...
      def canExecute(m: Option[Match], v: Map[String, Int]): Boolean = m match {
        case None => false
        case Some(m) => v.contains(m.group(1)) || isNumeric(m.group(1))
      }
    }

    case class TwoOpWire(f: (Int, Int) => Int) extends Wire {
      def exec(m: Match, v: Map[String, Int]): (String, Int) = (m.group(3), f(toInt(m.group(1), v), toInt(m.group(2), v)))
      def canExecute(m: Option[Match], v: Map[String, Int]): Boolean = m match {
        case None => false
        case Some(m) => (v.contains(m.group(1)) || isNumeric(m.group(1))) && (v.contains(m.group(2)) || isNumeric(m.group(2)))
      }
    }

    case class Operation(regex: String, wire: Wire) {
      def canExecute(l: String, v: Map[String, Int]): Boolean = wire.canExecute(regex.r.findFirstMatchIn(l), v)
      def execute(l: String, v: Map[String, Int]): (String, Int) = wire.exec(regex.r.findFirstMatchIn(l).get, v)
    }
    ...
    def getValues(ls: Seq[String], v: Map[String, Int]): (Seq[String], Map[String, Int]) = {
      if (ls.isEmpty) (ls, v)
        ...
        val pairs = ls
          .foldLeft(List[(String, Operation)]()) {
            case (ll, command) => os.find(_.canExecute(command, v)) match {
              case Some(op) => ll :+ (command, op)
              case _ => ll
            }
          }
        ...
        if (pairs.isEmpty) (ls, v)
          ...
          v ++ pairs.map {
            case (l, o) => o.execute(l, v)
          })
      }
    }

    val (nextLines, initialMap) = getValues(lines, Map())
    getValues(nextLines, initialMap) match {
      case (_, map) => map
    }
  }
}