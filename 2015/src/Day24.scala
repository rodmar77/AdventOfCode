...
/*
  ...
  side. Only when all three groups weigh exactly the same amount will the sleigh be
...
object Day24 extends App {
  ...
  def result(groupCount: Int) = {
    def partition(partitionSize: Int): BigInt = {
      if (partitionSize == data.size) 0
      else {
        val all = data
  ...
  of packages in the first group: 11 4, 10 5, and 8 7. Of these, 11 4 has the
