...
/*
  ...
  Instead, they count from a value called the relative base. The relative base starts
  at 0.
  ...
  Your Intcode computer will also need a few other capabilities:
    ...
    written like any other memory. (It is invalid to try to access memory at a negative
    address, though.)
  ...
  value 1. It will perform a series of checks on each opcode, output any opcodes (and the
  associated parameter modes) that seem to be functioning incorrectly, and finally output
  a BOOST keycode.
  ...
  Once run, it will boost the sensors automatically, but it might take a few seconds
  to complete the operation on slower hardware. In sensor boost mode, the program will
...
object Day09 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day09.txt")) {
      source =>
        val numbers = source
          .mkString
          .split(",")
        ...
        List(
          ...
          2)
          .foreach(value => {
            val output = ListBuffer.empty[BigInt]
            val intCodeMachine = IntCodeMachine(output.append)
            intCodeMachine.start(numbers)
            intCodeMachine.resume(value)
