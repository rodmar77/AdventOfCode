...
/*
  ...
  and E to 0. Then, you could determine the output signal that gets sent from amplifier E
  ...
  connected to amplifier B's input, and so on. However, the output from amplifier E is now
  connected into amplifier A's input. This creates the feedback loop: the signal will be
  ...
  halting. Provide each amplifier its phase setting at its first input instruction; all
  ...
  loop. When this happens, the last output signal from amplifier E is sent to the
...
object Day07 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day07.txt")) {
      source =>
        val numbers = source
          .mkString
          .split(",")
        ...
        println(getMaxThrust(numbers))

        // Try every combination of the new phase settings on the amplifier feedback loop.
        // What is the highest signal that can be sent to the thrusters?
        println(getMaxLoopedThrust(numbers))
    }
  }

  def getMaxThrust(p: List[BigInt]): BigInt = {
    def getThrust(n: IndexedSeq[Int]): BigInt = {
      n.foldLeft(BigInt(0)) {
        case (current, previous) =>
          val lb = ListBuffer[BigInt]()
          val machine = IntCodeMachine(lb.append)
          machine.start(p)
          machine.resume(previous, current)
          lb.last
      }
    }

    (0 to 4).permutations.map(getThrust).max
  }

  def getMaxLoopedThrust(p: List[BigInt]): BigInt = {
    def getLoopedThrust(n: Seq[Int]): BigInt = {
      @tailrec
      def loop(machines: Seq[(ListBuffer[BigInt], IntCodeMachine)], lastSignal: BigInt): BigInt = machines.last match {
        case (_, lastMachine) =>
          if (lastMachine.isRunning) loop(machines, machines.foldLeft(lastSignal) {
            case (current, (lb, machine)) =>
              machine.resume(current)
              lb.last
          })
          else lastSignal
      }

      val machines = (0 until 5).map(_ => {
        val lb = ListBuffer[BigInt]()
        val machine = IntCodeMachine(lb.append)
        (lb, machine)
      })

      val lastSignal = n.zip(machines).foldLeft(BigInt(0)) {
        case (current, (previous, (lb, machine))) =>
          machine.start(p)
          machine.resume(previous, current)
          lb.last
      }

      loop(machines, lastSignal)
    }

    (5 to 9).permutations.map(getLoopedThrust).max
  }
}
