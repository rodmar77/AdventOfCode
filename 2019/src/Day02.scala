...
object Day02 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day02.txt")) {
      source =>
        val numbers = source.mkString.split(",").map(BigInt(_)).toList
        ...
        println(intCodeMachine.start(numbers.updated(1, BigInt(12)).updated(2, BigInt(2)))(0))

        // Find the input noun and verb that cause the program to produce the output
        // 19690720. What is 100 * noun + verb? (For example, if noun=12 and verb=2,
        // the answer would be 1202.)
        println(produces(numbers, 19690720) match {
          case (noun, verb) => noun*100 + verb
        })
    }
  }

  def produces(program: List[BigInt], expected: BigInt): (BigInt, BigInt) = {
    (0 to 99).combinations(2).flatMap(_.permutations).find {
      case Seq(noun, verb) =>
        val intCodeMachine = IntCodeMachine()
        intCodeMachine.start(program.updated(1, BigInt(noun)).updated(2, BigInt(verb))) match {
          case program => program(0) == expected
        }
    } match {
      case Some(Seq(noun, verb)) => (noun, verb)
    }
  }
