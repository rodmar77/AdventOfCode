...
/*
  ...
  monitoring station. A monitoring station can detect any asteroid to which it has direct
  line of sight - that is, there cannot be another asteroid exactly between them. This
  line of sight can be at any angle, not just lines aligned to the grid or diagonally.
  ...
  another one detectable, the newly-detected asteroid won't be vaporized until the laser
...
object Day10 {
  ...
  case class Position(x: Int, y: Int) {
    def angle(that: Position): Angle = (this, that) match {
      case (Position(x0, y0), Position(x1, y1)) => atan2(x1-x0, y0-y1) * 180 / PI match {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day10.txt")) {
      source =>
        val coords = source.getLines().zipWithIndex.flatMap {
          case (line, y) => line.zipWithIndex.map {
            case (c, x) => (c, Position(x, y))
          }
        }.filter {
          case (c, _) => c == '#'
        }.map {
        ...
        calculateMostVisible(coords) match {
          case (position, visible) =>
            ...
            println(getXthAsteroid(coords, position, 200) match {
              case Position(x, y) => 100*x + y
            })
        }
    }
  }

  def getXthAsteroid(coords: List[Position], referencePosition: Position, explosionIndex: Int): Position = {
    @tailrec
    def getXthAsteroid(acc: Int, index: Int, angles: List[(Position, Angle)], current: Option[(Position, Angle)]): Option[(Position, Angle)] = {
      if (acc == explosionIndex || angles.isEmpty) current
      else if (index >= angles.size) getXthAsteroid(acc, 0, angles, None)
      else (current, angles(index)) match {
        case (Some((_, a)), (_, b)) if a == b => getXthAsteroid(acc, index + 1, angles, current)
        case _  => getXthAsteroid(acc + 1, index, angles.patch(index, Nil, 1), Option(angles(index)))
      }
    }

    val angles = coords
      .filterNot(_ == referencePosition)
      .map(p => (referencePosition, p, referencePosition.angle(p)))
      .sortBy {
        case (Position(x0, y0), Position(x1, y1), a) => (a, (x0 - x1).abs + (y0 - y1).abs)
      }.map {
        case (_, p, a) => (p, a)
      }

    getXthAsteroid(1, 0, angles.tail, Option(angles.head)) match {
      case Some((p, _)) => p
    }
  }

  def calculateMostVisible(coords: List[Position]): (Position, Int) = coords
      .map(a => (a, coords.filterNot(_ == a).map(_.angle(a)).toSet.size))
      .maxBy {
        case (_, visible) => visible
      }
