...
/*
  ...
  decide to start by tracking the four largest moons: Io, Europa, Ganymede, and Callisto.
  ...
  velocity changes by +1 (because 5 > 3) and Callisto's x velocity changes by -1 (because
  3 < 5). However, if the positions on a given axis are the same, the velocity on that
  ...
  velocity of x=-2, y=0,z=3, then its new position would be x=-1, y=2, z=6. This process
  ...
  velocity coordinates. Below, each line shows the calculations for a moon's potential
...
object Day12 {
  ...
  type MoonData = ((Int, Int, Int), (Int, Int, Int))

  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day12.txt")) {
      source =>
        val data = getData(source.getLines())

        // What is the total energy in the system after simulating the moons given in your
        // scan for 1000 steps?
        println(
          applySteps(data, 1000).map {
            case ((x, y, z), (vx, vy, vz)) => (x.abs + y.abs + z.abs) * (vx.abs + vy.abs + vz.abs)
          }.sum)

        // How many steps does it take to reach the first state that exactly matches a
        // previous state?
        println(getRepeatCycleLength(data))
    }
  }

  def getRepeatCycleLength(source: List[MoonData]): BigInt = {
    def matches(data: List[MoonData], dimension: Char) = data.zip(source).forall {
      case (((x0, y0, z0), (vx0, vy0, vz0)), ((x1, y1, z1), (vx1, vy1, vz1))) => dimension match {
        case 'x' => x0 == x1 && vx0 == vx1
        case 'y' => y0 == y1 && vy0 == vy1
        case 'z' => z0 == z1 && vz0 == vz1
      }
    }
    ...
    @tailrec
    def getRepeatCycleLength(acc: Int, xLength: Int, yLength: Int, zLength: Int, data: List[MoonData]): BigInt = {
      if (xLength > 0 && yLength > 0 && zLength > 0) lcm(xLength, yLength, zLength)
      else if (xLength == 0 && matches(data, 'x')) getRepeatCycleLength(acc, acc, yLength, zLength, data)
      else if (yLength == 0 && matches(data, 'y')) getRepeatCycleLength(acc, xLength, acc, zLength, data)
      else if (zLength == 0 && matches(data, 'z')) getRepeatCycleLength(acc, xLength, yLength, acc, data)
      else getRepeatCycleLength(acc + 1, xLength, yLength, zLength, applySteps(data, 1))
    }

    getRepeatCycleLength(1, 0, 0, 0, applySteps(source, 1))
  }

  @tailrec
  def applySteps(value: List[MoonData], steps: Int): List[MoonData] = {
    def changes(toCompare: MoonData, data: List[MoonData]): MoonData = {
      def calc(ck: Int, k: Int, rk: Int) = if (ck > k) rk + 1 else if (ck < k) rk - 1 else rk

      toCompare match {
        case ((x, y, z), (vx, vy, vz)) =>
          val (nx, ny, nz) = data.filterNot(_ == toCompare).foldLeft((x, y, z)) {
            case ((rx, ry, rz), ((cx, cy, cz), _)) =>
              (calc(cx, x, rx), calc(cy, y, ry), calc(cz, z, rz))
          }

          ((nx + vx, ny + vy, nz + vz), (nx + vx - x, ny + vy - y, nz + vz - z))
      }
    }

    def applyStep(data: List[MoonData]): List[MoonData] = data.map(changes(_, data))

    if (steps == 0) value
    else applySteps(applyStep(value), steps - 1)
  }

  def getData(lines: Iterator[String]): List[MoonData] = {
    val moonData = """<x=([-0-9]+), y=([-0-9]+), z=([-0-9]+)>""".r

    lines
      .map {
        case moonData(x, y, z) => ((x.toInt, y.toInt, z.toInt), (0, 0, 0))
      }.toList
  }
}
