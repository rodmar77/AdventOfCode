...
/*
  ...
  The Elves' spirits are lifted when they realize you have an opportunity to reboot one of
  their Mars rovers, and so they are curious if you would spend a brief sojourn on Mars.
  ...
  encoding; instead, they're encoded in a special Space Image Format. None of the Elves
  seem to remember why this is the case. They send you the instructions to decode it.
  ...
  pixel in the third layer, and a white pixel in the fourth layer, the final image would
  have a black pixel at that position.
  ...
  Then, the full image can be found by determining the top visible pixel in each position:
    ...
    layer is 1.
    The bottom-left pixel is white because the top two layers are 2, but the third layer
    is 1.
...
object Day08 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day08.txt")) {
      source =>
        val data = source.getLines().mkString
        val layers = data.toSeq.sliding(25, 25).map(_.unwrap).sliding(6, 6).toList

        val fewestZeroDigits = layers.minBy(_.mkString.count(_ == '0')).flatten

        // To make sure the image wasn't corrupted during transmission, the Elves would
        // like you to find the layer that contains the fewest 0 digits. On that layer,
        // what is the number of 1 digits multiplied by the number of 2 digits?
        println(fewestZeroDigits.count(_ == '1') * fewestZeroDigits.count(_ == '2'))

        // What message is produced after decoding your image?
        val x = (0 until 6).map(y => (0 until 25).map(x => layers.map(_(y)(x)).dropWhile(_ == '2').head))
