...
/*
  ...
  parameter to be interpreted as a position - if the parameter is 50, its value is the
  value stored at address 50 in memory. Until now, all parameters have been in position
  mode.
  ...
  not working correctly; check the instructions that were run before the output
  ...
  instruction pointer, that value is used and the instruction pointer is not automatically
  increased.
...
object Day05 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2019/input_day05.txt")) {
      source =>
        val numbers = source
          .mkString
          .split(",")
