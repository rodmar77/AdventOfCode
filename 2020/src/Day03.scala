...
object Day03 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2020/input_day03.txt")) {
      source => val grid = source
        .getLines
        .toList
      ...
      */
        ...
        println(
          List((1, 1), (3, 1), (5, 1), (7, 1), (1, 2)).map {
            case (right, down) => countTrees(grid, right, down)
          }.product)
  ...
  def countTrees(grid: List[String], right: Int, down: Int): Long = {
    ...
    countTrees(grid.drop(down), right, 0)
  }
}
