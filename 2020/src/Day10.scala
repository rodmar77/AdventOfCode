...
object Day10 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2020/input_day10.txt")) {
      ...
      */
        println(countArrangements(numbers))
    }
  }

  def countArrangements(numbers: List[Int]): BigInt = {
    def memoize[I, O](f: I => O): I => O = new mutable.HashMap[I, O]() {
      override def apply(key: I): O = getOrElseUpdate(key, f(key))
    }

    val x = numbers.size
    lazy val arrangements: ((Int, Int)) => BigInt = memoize {
      case (_, `x`) => 1
      case (current, si) => numbers
                              ...
                              .map {
                                case (number, index) => arrangements(number, index + 1)
                              }.sum
    }
