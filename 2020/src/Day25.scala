...
object Day25 {
  ...
  def main(args: Array[String]): Unit = {
    Using(Source.fromFile("inputs/2020/input_day25.txt")) {
      src => {
        val List(cpk, dpk) = src.getLines.toList.map(_.toLong)

        // What encryption key is the handshake trying to establish?
        println(transform(dpk, getLoopSize(cpk)))
      }
    }
  }

  def transform(n: Long, l: Int): Long = {
    @tailrec
    def _transform(v: Long, i: Int ): Long = {
      if (i == l) v
      else _transform((n * v) % 20201227, i + 1)
    }

    _transform(n, 1)
  }

  def getLoopSize(l: Long): Int = {
    @tailrec
    def getLoopSize(acc: Int, v: Long): Int = {
      if (v == l) acc
      else getLoopSize(acc + 1, (v * 7) % 20201227)
    }
