...
object Day15 {
  ...
  def main(args: Array[String]): Unit = {
    val discRegex = ".+ #(\\d+).+ (\\d+).+=\\d+.+ (\\d+)[.]".r
    val discs = Source
      .fromFile("inputs/2016/input_day15.txt")
      .getLines
      .map {
        case discRegex(index, posCount, startPos) => (index.toInt, posCount.toInt, startPos.toInt)
      }.toList
    ...
    */
    println(solve(discs :+ (discs.size + 1, 11, 0)))
  }

  def solve(ps: List[(Int, Int, Int)]): Int = {
    def solve(currTime: Int): Int = {
      def fallsThrough(ps: List[(Int, Int, Int)]): Boolean = {
        if (ps.isEmpty) true
        else ps.head match {
          case (index, posCount, startPos) =>
            ((startPos + index + currTime) % posCount == 0) && fallsThrough(ps.tail)
        }
      }

      if (fallsThrough(ps)) currTime
      else solve(currTime + 1)
    }
